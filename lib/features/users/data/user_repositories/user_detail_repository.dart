...
abstract class UserDetailRepository {
  Future<Either<AppError, UserDetailModel>> getUserData(int id);
}
...
class UserDetailRepositoryImpl extends UserDetailRepository {
  ...
  @override
  Future<Either<AppError, UserDetailModel>> getUserData(int id) async {
    try {
      final result = await _userDetailDataSource.getUserDetail(id);
      return Right(result);
    } on DioException catch (e) {
...
final userDetailRepoProvider = Provider<UserDetailRepository>((ref) {
  return UserDetailRepositoryImpl(ref.watch(userDetailsSourceProvider));
});
